...
/**
 * New-booking notification to the site owner. Same no-op rule as the booker email:
 * only sends if RESEND_API_KEY is set. Reply-to is the booker, so a reply goes
...
export async function sendOwnerNotification(opts: {
  ...
  const to = process.env.BOOKING_OWNER_EMAIL || BRAND.email
  const from = process.env.BOOKING_FROM_EMAIL || `${BRAND.name} <onboarding@resend.dev>`
  const resend = new Resend(key)

  const html = `
    <div style="font-family:-apple-system,Segoe UI,sans-serif;max-width:520px;margin:0 auto;color:#0f172a">
      <h2 style="font-size:20px">New booking: ${opts.meetingLabel}</h2>
      <p style="font-size:16px;font-weight:600">${opts.whenLabel}</p>
      <p>
        <strong>${opts.bookerName}</strong> (${opts.bookerEmail})<br/>
        Company: ${opts.company || "none"}
      </p>
      <p style="white-space:pre-wrap"><strong>Notes:</strong> ${opts.notes || "none"}</p>
      ${
        opts.meetLink
          ? `<p><a href="${opts.meetLink}" style="display:inline-block;background:#0f172a;color:#fff;padding:10px 18px;border-radius:8px;text-decoration:none">Join the Google Meet</a></p>`
          : ""
  ...
  try {
    await resend.emails.send({
      from,
      to,
      replyTo: opts.bookerEmail,
      subject: `New booking: ${opts.meetingLabel} (${opts.bookerName})`,
      html,
    })
  } catch (err) {
    // Never fail a booking because the notification email bounced.
